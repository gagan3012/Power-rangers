...
ranger_color(pink, magenta).  % magenta looks like pink
...
can_use_special :-
    player_special(Count),
    MaxSpecial = 3,
    Count < MaxSpecial,
    NewCount is Count + 1,
    retract(player_special(Count)),
    assert(player_special(NewCount)).
...
mission3_red :-
    ...
    prompt_action(wall_jump, R),
    ( R = success ->
         colored_format(yellow, 'After your action, ~w HP is now ~w~n', [bluehead, _]),  % HP details printed later
...
mission4_shadow :-
    ...
    ( R1 = success, R2 = success, R3 = success ->
         ...
         ( Outcome = win -> colored_writeln(green, 'Shadow Ranger: Ringbah’s Robot defeated!')
...
mission5_blue :-
    ...
    ( R = success ->
         colored_format(green, 'You attack dealing ~w damage!~n', [/*Damage computed in battle*/0]),
...
cool_battle([EnemyName|Rest], Outcome) :-
    ...
    nl,
    color_format(magenta, 'A wild ~w appears! (HP: ~w, Attack: ~w)~n', [EnemyName, EnemyHP, EnemyAttack]),
...
battle_loop(EnemyName, EnemyHP, EnemyAttack, Outcome) :-
    ...
    ; EnemyHP =< 0 ->
         Outcome = win,
         color_format(green, 'You defeated ~w!~n', [EnemyName])
    ; 
         % HP details in yellow:
...
% Processes the player’s chosen battle action.
...
player_turn(special, _EnemyName, EnemyHP, _EnemyAttack, NewEnemyHP, normal) :-
    ( can_use_special ->
         random_between(30, 40, Damage),
         colored_format(green, 'You unleash a special attack dealing ~w damage!~n', [Damage]),
         NewEnemyHP is EnemyHP - Damage
    ; 
         colored_writeln(white, 'You have no special attacks left! You lose your turn.'),
...
player_turn(_, _EnemyName, EnemyHP, _EnemyAttack, EnemyHP, none) :-
    colored_writeln(red, 'Invalid action, you lose your turn.').
...
% Updates the player's HP (caps at 100).
